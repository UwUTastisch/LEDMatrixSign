...
// fit the matrix.
// The firmware buffers an upload body in RAM, and base64 inflates a bitmap by
// a third, so the ceiling here is deliberately well under the device's 64 KB
// body cap. An ESP32-C3 has far less heap to spare than an S3, and a
// contiguous 59 KB allocation is not something to count on.
//
// A full-panel asset is always allowed; beyond that the limit is 24 KB of
// bitmap (about 32 KB of request body).
const MIN_ASSET_BUDGET = 24000;
const MAX_ASSET_BYTES_CEILING = 44000;
const MAX_ASSET_DIM = 4096;

function assetByteBudget() {
  const panel = state.width * state.height * 4 + 54;
  return clamp(panel, MIN_ASSET_BUDGET, MAX_ASSET_BYTES_CEILING);
}

function assetByteLimitSize(w, h) {
  const budget = Math.floor((assetByteBudget() - 54) / 4);
...
// A 32-bit RGBA pixel editor. Everything here is full-colour with alpha; the
// sign's asset format is BGRA8888, so nothing is quantised on the way out.

const AE_PALETTE_SLOTS = 20;
const AE_DEFAULT_PALETTE = [
  '#000000', '#ffffff', '#ff0000', '#ff8800', '#ffff00', '#88ff00',
  '#00ff00', '#00ff88', '#00ffff', '#0088ff', '#0000ff', '#8800ff',
  '#ff00ff', '#ff0088', '#884400', '#888888', '#444444', '#ffccaa',
  '#224466', '#113322',
];
...
const assetEditor = {
  w: 16,
  h: 16,
  px: new Uint8ClampedArray(16 * 16 * 4),
  tool: 'brush',
  size: 1,
  color: { r: 255, g: 0, b: 0 },
  alpha: 255,
  palette: AE_DEFAULT_PALETTE.slice(),
  paletteIndex: 2,
  cell: 12,
  stroke: null, // in-progress drag: { mode, from, before }
  undo: [],
  redo: [],
};

// ---------- persistence (the palette outlives a reload) ----------

function aeLoadPalette() {
  try {
    const raw = window.localStorage.getItem('ledmatrix.palette');
    const list = raw ? JSON.parse(raw) : null;
    if (Array.isArray(list) && list.length === AE_PALETTE_SLOTS) {
      assetEditor.palette = list.map((c) => (parseHex(c) ? c : '#000000'));
    }
  } catch (e) {
    // Private mode, or a file:// page with storage disabled — defaults are fine.
  }
}

function aeSavePalette() {
  try {
    window.localStorage.setItem('ledmatrix.palette', JSON.stringify(assetEditor.palette));
  } catch (e) {
    // Not worth telling anyone about; the palette just won't persist.
  }
}

// ---------- pixel helpers ----------

function aeIndex(x, y) {
  return (y * assetEditor.w + x) * 4;
}

function aeInside(x, y) {
  return x >= 0 && y >= 0 && x < assetEditor.w && y < assetEditor.h;
}

function aeCurrentRgba() {
  const { color, alpha } = assetEditor;
  return [color.r, color.g, color.b, alpha];
}

function aeSetPixel(x, y, rgba) {
  if (!aeInside(x, y)) return;
  const i = aeIndex(x, y);
  assetEditor.px[i] = rgba[0];
  assetEditor.px[i + 1] = rgba[1];
  assetEditor.px[i + 2] = rgba[2];
  assetEditor.px[i + 3] = rgba[3];
}

function aeGetPixel(x, y) {
  const i = aeIndex(x, y);
  return [assetEditor.px[i], assetEditor.px[i + 1], assetEditor.px[i + 2], assetEditor.px[i + 3]];
}

// A square nib, like the 1.0 tool's brush size.
function aeStamp(x, y, rgba) {
  const n = clamp(assetEditor.size, 1, 16);
  const half = Math.floor((n - 1) / 2);
  for (let j = 0; j < n; j += 1) {
    for (let i = 0; i < n; i += 1) aeSetPixel(x - half + i, y - half + j, rgba);
  }
}

function aeLineStamp(from, to, rgba) {
  let { x, y } = from;
...
function aeRectStamp(from, to, rgba) {
  const x0 = Math.min(from.x, to.x);
  const x1 = Math.max(from.x, to.x);
  const y0 = Math.min(from.y, to.y);
  const y1 = Math.max(from.y, to.y);
  aeLineStamp({ x: x0, y: y0 }, { x: x1, y: y0 }, rgba);
  aeLineStamp({ x: x0, y: y1 }, { x: x1, y: y1 }, rgba);
  aeLineStamp({ x: x0, y: y0 }, { x: x0, y: y1 }, rgba);
  aeLineStamp({ x: x1, y: y0 }, { x: x1, y: y1 }, rgba);
}

// Flood fill over pixels that match the starting one exactly (4-connected).
function aeBucket(x, y, rgba) {
  const target = aeGetPixel(x, y);
  const same = (p) => p[0] === target[0] && p[1] === target[1] && p[2] === target[2] && p[3] === target[3];
  if (same(rgba)) return;
  const stack = [[x, y]];
  const seen = new Uint8Array(assetEditor.w * assetEditor.h);
  while (stack.length) {
    const [cx, cy] = stack.pop();
    if (!aeInside(cx, cy)) continue;
    const key = cy * assetEditor.w + cx;
    if (seen[key]) continue;
    seen[key] = 1;
    if (!same(aeGetPixel(cx, cy))) continue;
    aeSetPixel(cx, cy, rgba);
...
function aeSnapshot() {
  return { w: assetEditor.w, h: assetEditor.h, px: new Uint8ClampedArray(assetEditor.px) };
}

function aeRestore(snap) {
  assetEditor.w = snap.w;
  assetEditor.h = snap.h;
  assetEditor.px = new Uint8ClampedArray(snap.px);
  $('aeWidth').value = String(snap.w);
  $('aeHeight').value = String(snap.h);
}

function aeCommit(before) {
  assetEditor.undo.push(before);
  if (assetEditor.undo.length > 60) assetEditor.undo.shift();
  assetEditor.redo = [];
  aeUpdateHistoryButtons();
}

function aeUpdateHistoryButtons() {
  $('aeUndo').disabled = assetEditor.undo.length === 0;
  $('aeRedo').disabled = assetEditor.redo.length === 0;
}

function aeUndoStep() {
  const snap = assetEditor.undo.pop();
  if (!snap) return;
  assetEditor.redo.push(aeSnapshot());
  aeRestore(snap);
  aeUpdateHistoryButtons();
  renderAssetEditor();
}
...
function aeRedoStep() {
  const snap = assetEditor.redo.pop();
  if (!snap) return;
  assetEditor.undo.push(aeSnapshot());
  aeRestore(snap);
  aeUpdateHistoryButtons();
  renderAssetEditor();
}
...
function resizeAssetEditor(w, h, keepHistory) {
  const nw = clamp(Math.round(num(w, 16)), 1, 256);
  const nh = clamp(Math.round(num(h, 16)), 1, 256);
  if (nw === assetEditor.w && nh === assetEditor.h) return;
  if (!keepHistory) aeCommit(aeSnapshot());
...
// Crop fully transparent rows and columns off the edges.
function aeTrim() {
  let minX = assetEditor.w;
  let minY = assetEditor.h;
  let maxX = -1;
  let maxY = -1;
  for (let y = 0; y < assetEditor.h; y += 1) {
    for (let x = 0; x < assetEditor.w; x += 1) {
      if (assetEditor.px[aeIndex(x, y) + 3] === 0) continue;
      if (x < minX) minX = x;
      if (y < minY) minY = y;
      if (x > maxX) maxX = x;
      if (y > maxY) maxY = y;
  ...
  if (maxX < 0) {
    toast('Nothing to trim — the canvas is empty.', 'error');
    return;
  }
  const nw = maxX - minX + 1;
  const nh = maxY - minY + 1;
  if (nw === assetEditor.w && nh === assetEditor.h) {
    toast('No transparent edges to trim.');
    return;
  }
  aeCommit(aeSnapshot());
  const next = new Uint8ClampedArray(nw * nh * 4);
  for (let y = 0; y < nh; y += 1) {
    const s = aeIndex(minX, minY + y);
    next.set(assetEditor.px.subarray(s, s + nw * 4), y * nw * 4);
  }
  assetEditor.w = nw;
  assetEditor.h = nh;
  assetEditor.px = next;
  ...
  $('aeHeight').value = String(nh);
  renderAssetEditor();
  toast(`Trimmed to ${nw} × ${nh}`);
}

function renderAssetEditor() {
  const cv = $('assetEditorCanvas');
  const { w, h } = assetEditor;
  const cell = clamp(Math.floor(Math.min(520 / w, 380 / h)), 3, 24);
  ...
  const note = $('aeSizeNote');
  const bytes = 54 + w * h * 4;
  note.textContent = `${w} × ${h} — ${formatBytes(bytes)}`;
  note.classList.toggle('warn', bytes > assetByteBudget());
  if (bytes > assetByteBudget()) note.textContent += ' — too large for the sign';
}

// ---------- palette + colour ----------

function aeRenderPalette() {
  const host = $('aePalette');
  host.innerHTML = '';
  assetEditor.palette.forEach((color, i) => {
    const b = document.createElement('button');
    b.type = 'button';
    b.className = `ae-swatch${i === assetEditor.paletteIndex ? ' selected' : ''}`;
    b.style.background = color;
    b.title = `${color} — shift-click to replace with the current color`;
    b.addEventListener('click', (event) => {
      if (event.shiftKey) {
        assetEditor.palette[i] = aeColorHex();
        aeSavePalette();
      } else {
        assetEditor.paletteIndex = i;
        aeSetColor(assetEditor.palette[i]);
      }
      aeRenderPalette();
    });
    host.appendChild(b);
...
function aeColorHex() {
  const { r, g, b } = assetEditor.color;
  return `#${[r, g, b].map((v) => v.toString(16).padStart(2, '0')).join('')}`;
}

function aeSetColor(hex) {
  const c = parseHex(hex);
  if (!c) return;
  assetEditor.color = { r: c.r, g: c.g, b: c.b };
  $('aeColor').value = aeColorHex();
}

function aeSetAlpha(value) {
  assetEditor.alpha = clamp(Math.round(num(value, 255)), 0, 255);
  $('aeAlpha').value = String(assetEditor.alpha);
  $('aeAlphaValue').textContent = `${Math.round((assetEditor.alpha / 255) * 100)}%`;
}

// Fill the palette with the colours actually used, most common first.
function aePaletteFromImage() {
  const counts = new Map();
  for (let i = 0; i < assetEditor.px.length; i += 4) {
    if (assetEditor.px[i + 3] === 0) continue;
    const key = (assetEditor.px[i] << 16) | (assetEditor.px[i + 1] << 8) | assetEditor.px[i + 2];
    counts.set(key, (counts.get(key) || 0) + 1);
  }
  if (!counts.size) {
    toast('The canvas is empty, so there are no colors to collect.', 'error');
    return;
  }
  const top = [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, AE_PALETTE_SLOTS);
  const hex = (k) => `#${(k & 0xffffff).toString(16).padStart(6, '0')}`;
  assetEditor.palette = top.map(([k]) => hex(k));
  while (assetEditor.palette.length < AE_PALETTE_SLOTS) {
    assetEditor.palette.push(AE_DEFAULT_PALETTE[assetEditor.palette.length]);
  }
  assetEditor.paletteIndex = 0;
  aeSetColor(assetEditor.palette[0]);
  aeSavePalette();
  aeRenderPalette();
  toast(`Palette filled with ${Math.min(top.length, AE_PALETTE_SLOTS)} colors from the image`);
}

function setAssetTool(tool) {
  assetEditor.tool = tool;
  document.querySelectorAll('.ae-tool').forEach((b) => b.classList.toggle('active', b.dataset.aeTool === tool));
  const cv = $('assetEditorCanvas');
  cv.classList.toggle('picking', tool === 'picker');
}

// ---------- pointer handling ----------

function aeCellAt(event) {
  const cv = $('assetEditorCanvas');
...
// Right-click picks a colour and middle-click erases, whatever tool is active.
function aeEffectiveTool(event) {
  if (event.button === 2) return 'picker';
  if (event.button === 1) return 'eraser';
  return assetEditor.tool;
}

function aePickColor(cell) {
  const [r, g, b, a] = aeGetPixel(cell.x, cell.y);
  if (a === 0) {
    toast('That pixel is transparent.', 'error');
    return;
  }
  assetEditor.color = { r, g, b };
  $('aeColor').value = aeColorHex();
  aeSetAlpha(a);
  assetEditor.paletteIndex = -1;
  aeRenderPalette();
}

function onAePointerDown(event) {
  const cv = $('assetEditorCanvas');
  const cell = aeCellAt(event);
  const tool = aeEffectiveTool(event);
  ...
  const before = aeSnapshot();
  const rgba = tool === 'eraser' ? [0, 0, 0, 0] : aeCurrentRgba();

  if (tool === 'bucket') {
    aeBucket(cell.x, cell.y, rgba);
    aeCommit(before);
    renderAssetEditor();
    return;
  }

  assetEditor.stroke = { tool, rgba, from: cell, last: cell, before };
  if (tool === 'brush' || tool === 'eraser') aeStamp(cell.x, cell.y, rgba);
  renderAssetEditor();
}

function onAePointerMove(event) {
  const stroke = assetEditor.stroke;
  if (!stroke) return;
  const cell = aeCellAt(event);
  if (stroke.tool === 'brush' || stroke.tool === 'eraser') {
    // Interpolate so a fast drag leaves no gaps.
    aeLineStamp(stroke.last, cell, stroke.rgba);
    stroke.last = cell;
  } else {
    // Line and rectangle preview from the untouched snapshot.
    assetEditor.px = new Uint8ClampedArray(stroke.before.px);
    if (stroke.tool === 'line') aeLineStamp(stroke.from, cell, stroke.rgba);
    else aeRectStamp(stroke.from, cell, stroke.rgba);
    stroke.last = cell;
  }
  renderAssetEditor();
}

function onAePointerUp() {
  const stroke = assetEditor.stroke;
  assetEditor.stroke = null;
  if (!stroke) return;
  aeCommit(stroke.before);
  renderAssetEditor();
}

// ---------- importing bitmaps ----------

// Draw an image into the canvas. With `resize` the canvas takes the image's
// size (capped to what the sign can hold); otherwise the image is scaled to
// fit the current canvas.
async function aeImportImage(source, { resize } = {}) {
  let bitmap;
  try {
    bitmap = await createImageBitmap(source);
  } catch (e) {
    // BMPs that browsers refuse (32-bit BGRA with BI_BITFIELDS, say) still
    // decode with our own reader.
    try {
      const buffer = source instanceof Blob ? await source.arrayBuffer() : source;
      const decoded = decodeBmp(buffer);
      const tmp = new Raster(decoded.w, decoded.h);
      tmp.px.set(decoded.rgba);
      bitmap = await createImageBitmap(tmp.toCanvas());
    } catch (e2) {
      throw new Error(`Couldn't read that image: ${e.message}`);
    }
  }
  ...
  if (resize) {
    tw = bitmap.width;
    th = bitmap.height;
    // An asset larger than the panel can never be shown in full, so the
    // default is to shrink to the sign; otherwise only the size the sign can
    // actually store applies.
    if ($('aeFitSign').checked && (tw > state.width || th > state.height)) {
      const scale = Math.min(state.width / tw, state.height / th);
      tw = Math.max(1, Math.round(tw * scale));
      th = Math.max(1, Math.round(th * scale));
    }
    const fit = assetByteLimitSize(tw, th);
    if (fit) {
      tw = fit.w;
      th = fit.h;
    }
    ...
    assetEditor.w = tw;
    assetEditor.h = th;
    assetEditor.px = new Uint8ClampedArray(tw * th * 4);
    $('aeWidth').value = String(tw);
    $('aeHeight').value = String(th);
  } else {
    const scale = Math.min(assetEditor.w / bitmap.width, assetEditor.h / bitmap.height, 1);
    tw = Math.max(1, Math.round(bitmap.width * scale));
    th = Math.max(1, Math.round(bitmap.height * scale));
  }
  ...
  cv.height = th;
  const c = cv.getContext('2d');
  const shrinking = tw < bitmap.width || th < bitmap.height;
  c.imageSmoothingEnabled = shrinking && $('aeSmooth').checked;
  c.imageSmoothingQuality = 'high';
  c.drawImage(bitmap, 0, 0, tw, th);
  const data = c.getImageData(0, 0, tw, th).data;

  // Centre it when it's smaller than the canvas.
  const ox = Math.floor((assetEditor.w - tw) / 2);
  const oy = Math.floor((assetEditor.h - th) / 2);
  for (let y = 0; y < th; y += 1) {
    for (let x = 0; x < tw; x += 1) {
      const s = (y * tw + x) * 4;
      aeSetPixel(ox + x, oy + y, [data[s], data[s + 1], data[s + 2], data[s + 3]]);
    }
  }
  aeCommit(before);
  renderAssetEditor();
  const scaled = tw !== bitmap.width || th !== bitmap.height;
  toast(scaled
    ? `Imported at ${tw} × ${th} (from ${bitmap.width} × ${bitmap.height})`
    : `Imported at ${tw} × ${th}`);
}

// An image on the clipboard or in a drop, if there is one.
function imageFromTransfer(source) {
  const items = source && (source.items || source.files);
  if (!items) return null;
  for (const item of items) {
    if (item.kind === 'file' && item.type.startsWith('image/')) return item.getAsFile();
    if (item instanceof File && item.type.startsWith('image/')) return item;
  }
  return null;
}
...
function aeSuggestName(base) {
  // Bump a trailing number so frames can be drawn one after another.
  const m = String(base || 'asset').match(/^(.*?)(\d+)$/);
  if (!m) return `${base}_1`;
  return `${m[1]}${Number(m[2]) + 1}`;
}

function aeReset(w, h) {
  assetEditor.w = w;
  assetEditor.h = h;
  assetEditor.px = new Uint8ClampedArray(w * h * 4);
  assetEditor.undo = [];
  assetEditor.redo = [];
  assetEditor.stroke = null;
  $('aeWidth').value = String(w);
  $('aeHeight').value = String(h);
  aeUpdateHistoryButtons();
}

function openAssetEditor(ref, options = {}) {
  setAssetTool('brush');
  ...
  if (!ref) {
    ...
    if (options.importFrom) {
      aeImportImage(options.importFrom, { resize: true }).catch(fail);
    }
    return;
  }
  const { anim, file } = splitAssetRef(ref);
  ensureAsset(ref).then((asset) => {
    aeReset(asset.w, asset.h);
...
function saveAssetFromEditor() {
  ...
  const bytes = 54 + assetEditor.w * assetEditor.h * 4;
  if (bytes > assetByteBudget()) {
    toast(`${assetEditor.w} × ${assetEditor.h} is ${formatBytes(bytes)}; this sign accepts up to ${formatBytes(assetByteBudget())} per asset. Shrink it or trim the edges.`, 'error');
    return;
  }
  ...
  uploadAssetBytes(anim, file, encodeBmp32(assetEditor.w, assetEditor.h, assetEditor.px))
    ...
    .then(() => {
      selectAsset(`${anim}/${file}`, false);
      // Ready for the next frame, without closing the editor.
      $('assetNameInput').value = aeSuggestName(file.replace(/\.bmp$/i, ''));
    })
    .catch(fail);
}
...
function initAssetEditor() {
  const cv = $('assetEditorCanvas');
  cv.addEventListener('pointerdown', onAePointerDown);
  cv.addEventListener('pointermove', onAePointerMove);
  cv.addEventListener('pointerup', onAePointerUp);
  cv.addEventListener('pointercancel', onAePointerUp);
  cv.addEventListener('contextmenu', (event) => event.preventDefault());

  document.querySelectorAll('.ae-tool').forEach((b) => {
    b.addEventListener('click', () => setAssetTool(b.dataset.aeTool));
  });
  $('aeBrushSize').addEventListener('input', (event) => {
    assetEditor.size = clamp(Math.round(num(event.target.value, 1)), 1, 16);
  });
  $('aeColor').addEventListener('input', (event) => {
    aeSetColor(event.target.value);
    assetEditor.paletteIndex = -1;
    aeRenderPalette();
  });
  $('aeAlpha').addEventListener('input', (event) => aeSetAlpha(event.target.value));
  $('aePaletteFromImage').addEventListener('click', aePaletteFromImage);
  $('aeUndo').addEventListener('click', aeUndoStep);
  $('aeRedo').addEventListener('click', aeRedoStep);
  $('aeClear').addEventListener('click', () => {
    aeCommit(aeSnapshot());
    assetEditor.px.fill(0);
    renderAssetEditor();
  });
  $('aeTrim').addEventListener('click', aeTrim);
  $('aeFitMatrix').addEventListener('click', () => resizeAssetEditor(state.width, state.height));
  ...
  $('aeImport').addEventListener('change', (event) => {
    const file = event.target.files[0];
    event.target.value = '';
    if (file) aeImportImage(file, { resize: $('aeImportResize').checked }).catch(fail);
  });
  ...
  stage.addEventListener('drop', (event) => {
    ...
    if (file) aeImportImage(file, { resize: $('aeImportResize').checked }).catch(fail);
    else toast('That drop had no image in it.', 'error');
  });

  syncAssetEditorLimits();
  aeLoadPalette();
  aeRenderPalette();
  aeSetColor(assetEditor.palette[assetEditor.paletteIndex] || '#ff0000');
  aeSetAlpha(255);
  renderAssetEditor();
}

// Paste anywhere on the page: into the editor if it's open, otherwise open it
// with the image loaded. This is the quickest path from a phone screenshot to
// an asset on the sign.
function initClipboardImport() {
  document.addEventListener('paste', (event) => {
    if (isTyping(event.target)) return;
    const file = imageFromTransfer(event.clipboardData);
    if (!file) return;
    event.preventDefault();
    if ($('assetEditorModal').classList.contains('hidden')) {
      switchTab('assets');
      openAssetEditor(null, { importFrom: file });
    } else {
      aeImportImage(file, { resize: $('aeImportResize').checked }).catch(fail);
    }
  });
}
...
function bindEvents() {
  ...
  // Dropping an image on the Assets tab opens the editor with it loaded.
  const assetsPanel = document.querySelector('[data-tab-panel="assets"]');
  ['dragenter', 'dragover'].forEach((type) => assetsPanel.addEventListener(type, (event) => {
    event.preventDefault();
    assetsPanel.classList.add('dropping');
  }));
  ['dragleave', 'drop'].forEach((type) => assetsPanel.addEventListener(type, (event) => {
    assetsPanel.classList.remove('dropping');
  }));
  assetsPanel.addEventListener('drop', (event) => {
    event.preventDefault();
    const file = imageFromTransfer(event.dataTransfer);
    if (file) openAssetEditor(null, { importFrom: file });
  });
  ...
  const AE_KEYS = { b: 'brush', e: 'eraser', f: 'bucket', l: 'line', r: 'rect', i: 'picker' };
  document.addEventListener('keydown', (event) => {
    // The asset editor has its own shortcuts while it's open.
    if (!$('assetEditorModal').classList.contains('hidden') && !isTyping(event.target)) {
      const k = event.key.toLowerCase();
      if (event.ctrlKey || event.metaKey) {
        if (k === 'z' && !event.shiftKey) { event.preventDefault(); aeUndoStep(); return; }
        if ((k === 'z' && event.shiftKey) || k === 'y') { event.preventDefault(); aeRedoStep(); return; }
      } else if (AE_KEYS[k]) {
        setAssetTool(AE_KEYS[k]);
        return;
      }
    }
    if (event.key === 'Escape') {
