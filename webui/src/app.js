...
function renderAssetEditor() {
  ...
  note.classList.toggle('warn', bytes > MAX_ASSET_BYTES);
  if (bytes > MAX_ASSET_BYTES) note.textContent += ' — too large for the sign';
}
...
function saveAssetFromEditor() {
  ...
  if (bytes > MAX_ASSET_BYTES) {
    toast(`${assetEditor.w} × ${assetEditor.h} is ${formatBytes(bytes)}; the sign accepts up to ${formatBytes(MAX_ASSET_BYTES)}. Shrink it or trim the edges.`, 'error');
    return;
  }
